 ...
 * convenience. <p> These static factory methods allow easy construction of
 * value objects using either long values like <code>seconds(2034)</code> or
 * <code>hours(3)</code>: <p> <ul> <li><code>Duration.milliseconds(long)</code>
 * <li><code>Duration.seconds(int)</code> <li><code>Duration.minutes(int)</code>
 * <li><code>Duration.hours(int)</code> <li><code>Duration.days(int)</code>
 * </ul> <p> ...or double-precision floating point values like
 * <code>days(3.2)</code>: <p> <ul>
 ...
 * <li><code>Duration.days(double)</code> </ul> <p> In the case of
 * <code>milliseconds(double)</code>, the value will be rounded off to the
 * nearest integral millisecond using <code>Math.round()</code>. <p> The precise
 * number of milliseconds represented by a <code>Duration</code> object can be
 * retrieved by calling the <code>getMilliseconds</code> method. The value of a
 * <code>Duration</code> object in a given unit like days or hours can be
 * retrieved by calling one of the following unit methods, each of which returns
 * a double-precision floating point number: <p> <ul> <li><code>seconds()</code>
 * <li><code>minutes()</code> <li><code>hours()</code> <li><code>days()</code>
 * </ul> <p> Values can be added and subtracted using the
 * <code>add(Duration)</code> and <code>subtract(Duration)</code> methods, each
 * of which returns a new immutable <code>Duration</code> object. <p>
 ...
 * Eurozone. <p> The benchmark method will "benchmark" a <code>Runnable</code>
 * or an {@link ICode} implementing object, returning a <code>Duration</code>
 * object that represents the amount of time elapsed in running the code. <p>
...
public class Duration extends AbstractTimeValue {
	...
	// * object. The string can take the form of a floating point number
	// followed
	// * by a number of milliseconds, seconds, minutes, hours or days. For
	// example
	...
	//
	// if (matcher.matches()) {
	// final double value = StringValue.valueOf(matcher.group(1),
	// locale).toDouble();
	...
	// }
	// } else {
	// throw new StringValueConversionException("Unable to parse duration: " +
	// string);
	// }
	// }
	...
	private String unitString(final double value, final String units, final Locale locale) {
		return valueOf(value, locale) + " " + units + (value > 1.0 ? "s" : "");
	}

	/* start of adaption from org.apache.wicket.util.string.StringValue */
	private static String valueOf(final double value, final Locale locale) {
		return valueOf(value, 1, locale);
	}

	private static String valueOf(final double value, final int places, final Locale locale) {
	...
	private static String repeat(final int times, final char c) {
		final StringBuilder buffer = new StringBuilder(times);
	...
	/* end of adaption from org.apache.wicket.util.string.StringValue */
}
